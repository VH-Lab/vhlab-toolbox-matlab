...
%   
%   [PARAMS,FVAL] = MLE_PoisGauss(STIM_CENTER_TIMES, STIM_DURATIONS, RESPONSES_OBSERVED, ...
...
% given r, g, and s, calculate the likelihood
	...
	r_estimated = response_gaindrift_model(t,dt,stimlist,SPs,g_params_value,stim_responses_i);
	%figure(100);
	%plot(t,r_estimated,'b'); 
	%pause(1);
	if strcmp(model,'Poisson'),
		y = -loglikelihood_spiketrain(r_estimated, dt, responses_observed);
	else,
		y = -loglikelihood_spikerate(r_estimated, dt, responses_observed, 1);
	end;


	% for debugging: %figure(100); %plot(t,r_estimated); %if y<357, %	x0, %end; %ans = -y; %global my_nested_workspace; %my_nested_workspace = workspace2struct;
