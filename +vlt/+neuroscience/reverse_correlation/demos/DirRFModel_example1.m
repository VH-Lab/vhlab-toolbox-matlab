...
% DIRRFMODEL_EXAMPLE1 - Direction selective responses from a hand-picked kernel
%
...
D = createdirkernel(Dx,Dt,SpFreq,0,TFreq,Left,[3 2],[0.1 0.001 0.2],0.050); % pretend mV
stim_left = stim1d_motion(Stimx,Stimt,SpFreq,0,TFreq,Left);
stim_right = stim1d_motion(Stimx,Stimt,SpFreq,0,TFreq,Right);

disp(['Setup complete, now simulating']);

[R_left,sim_time] = simulate_1dkernel_response(D, Dx,Dt, stim_left, Stimx, Stimt);
[R_right,sim_time] = simulate_1dkernel_response(D, Dx,Dt, stim_right, Stimx, Stimt);

disp(['Linear component simulated, now generating firing rates']);

Rrate_left = 1000*rectify(R_left); %.^3;  % now assume 1 Hz / mV depolarization
Rrate_right = 1000*rectify(R_right); %.^3;

disp(['Firing rates simulated, now generating spike times from these rates']);

sim_time_spikes = 0:0.001:Stimt(end);
Rrate_left_highsample = stepfunc(sim_time,Rrate_left,sim_time_spikes);
Rrate_right_highsample = stepfunc(sim_time,Rrate_right,sim_time_spikes);
