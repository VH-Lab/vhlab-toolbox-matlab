...
% 
% PARAMS = MLE_POISSON_VM_FR(VM, FR, DELTAT,  ...)
%
...
% For 'linearthreshold', PARAMS is [slope threshold] (see help LINEPOWERTHRESHOLD)
% For 'linepowerthreshold', PARAMS is [slope threshold exponent]
...
%   p_out =mle_poisson_vm_fr(0*fr,fr,deltaT,'fit_function','Poisson')  
...
switch lower(fit_function),
	...
	case 'linepowerthreshold',
		...
		rateFunc = 'linepowerthreshold';
		fitfun = @(x) -poisson_vm_fr(vm,fr,deltaT,rateFunc, {x(1) eps x(2) x(3)});
	otherwise,
