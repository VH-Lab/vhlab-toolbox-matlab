...
stim_onsetoffsetid_orig = stim_onsetoffsetid;
...
for i=1:size(stim_onsetoffsetid,1),
	stimpres(i) = vlt.data.findclosest(stim_onsetoffsetid_orig(:,1),stim_onsetoffsetid(i,1));
	...
	if isempty(f1),
		...
		vmobsperstim{i} = vo;
		frobsperstim{i} = firingrate_observations(indexes);
	else,
		si = timepoints(2)-timepoints(1); % sample interval
		mean_v(i) = abs(vlt.math.fouriercoeffs_tf2(vo, f1, 1/si));
		mean_fr(i) = abs(vlt.math.fouriercoeffs_tf2(firingrate_observations(indexes), f1, 1/si));
		mean_stimid(i) = stim_onsetoffsetid(i,3);
		vmobsperstim{i} = [];
		frobsperstim{i} = []
	end;
