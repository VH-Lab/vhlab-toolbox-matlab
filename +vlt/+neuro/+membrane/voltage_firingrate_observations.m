...
if ~isempty(vm_baseline_correct),
	...
	for i=1:size(stim_onsetoffsetid,1), % for each stimulus
		sample_start = vlt.signal.point2samplelabel(stim_onsetoffsetid(i,1),dt,t(1));
		if i<size(stim_onsetoffsetid,1),
			...
			sample_stop = vlt.signal.point2samplelabel(stim_onsetoffsetid(i+1,1),dt,t(1)) - vm_baseline_correct_samples - 1;
		else,
			sample_stop = numel(vm);
		end
		%sample_stop = vlt.signal.point2samplelabel(stim_onsetoffsetid(i,2),dt,t(1));
 ...
 % grab samples we need rather than calling vlt.math.slidingwindowfunc; these data sets could get large
...
for s = 1:numel(stimids),
	...
	for o=1:numel(do),
		sample_start = vlt.signal.point2samplelabel(stim_onsetoffsetid(do(o),1),dt,t(1));
		if do(o)<size(stim_onsetoffsetid,1),
			sample_stop = vlt.signal.point2samplelabel(stim_onsetoffsetid(do(o)+1,1)-dt-vm_baseline_correct,dt,t(1));
		else,
			sample_stop = min(vlt.signal.point2samplelabel(stim_onsetoffsetid(do(o),2)+median_ISI,dt,t(1)),numel(vm));
		end
