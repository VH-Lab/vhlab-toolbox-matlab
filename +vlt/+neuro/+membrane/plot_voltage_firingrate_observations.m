...
[newh,newhtext]=vlt.neuroscience.stimulus_analysis.plot_stimulus_timeseries(mean(v(inds)),stim_onsetoffsetid(:,1),...
...
figure;
[SI,INDS] = vlt.math.stepfunc(timepoints(:)'-0.5*binsize,fr(:)',t(:)');
plot(t(:),SI(:),'m');
xlabel('Time (s)');
...
[SI,INDS] = vlt.math.stepfunc(timepoints(:)'-0.5*binsize,v(:)',t(:)');
plot(t(:),SI(:),'m');
xlabel('Time (s)');
