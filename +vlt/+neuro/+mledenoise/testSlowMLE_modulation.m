...
[spiketimes,r,t,stimon,stimoff,stimids,g]=vlt.neuro.mledenoise.gaindriftexample('gain_amplitude',0.5,'stim_response',[30 25 20 15 10 5 0],...
...
RR_real = vlt.neuro.mledenoise.response_gaindrift_model(stim_center_times, stim_durations, stimids, real_sinparams, real_sinparams(1:3:12), real_s_i);
hold on;
plot((stimon+stimoff)/2, RR_real,'ko','DisplayName','True underlying responses s(t)*g(t) w/o noise');

spike_response = vlt.neuro.spiketrains.spikeresponse_tf(stimon,stimoff,stimids,spiketimes,dt,freq);
...
[s_i_params, g_params, sinparams, mle, responseobserved, stimlist, responses_s_i] = ...
	vlt.neuro.mledenoise.SlowMLEDeNoise(stimon, stimoff, stimids, spiketimes,'showfitting',1,'numIterations',6,...
...
hold on;
h1=vlt.plot.myerrorbar(1:length(real_s_i),abs(spike_response.curve(2,:)),abs(spike_response.curve(4,:)),abs(spike_response.curve(4,:)),'bs');
delete(h1(2));
h2=vlt.plot.myerrorbar(1:length(real_s_i),s_i_params,'ro');
delete(h2(2));
...
RR = vlt.neuro.mledenoise.response_gaindrift_model((stimon+stimoff)/2, 2, stimids, sinparams, g_params, s_i_params);
hold on;
