...
imwrite( vlt.math.rescale(score(:,1:25), max(abs(vlt.data.colvec(score(:,1:25))))*[-1 1], [0 255]), ...
	gray(256), [dirname filesep 'pca_projected.tif'] );

imwrite( (reshape(im_vector_mn,360,260)')', gray(256), [dirname filesep 'mean_face.tif'] );

for i=1:25,
	imwrite( vlt.math.rescale( (reshape(coeff(:,i),360,260)')', [min(coeff(:,2)) max(coeff(:,2))], [0 255]), ...
