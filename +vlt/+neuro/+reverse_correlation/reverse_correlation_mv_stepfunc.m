...
lags = vlt.signal.point2samplelabel(kerneltimes,dt_signal,dt_signal);

maxlag = max(abs(lags));

xc_stimsignal = vlt.signal.xcorr_stepfunc(signal_t,signal,maxlag,stimtimes,stim);
lagsteps = [-maxlag:1:maxlag];
xc_begin = vlt.data.findclosest(lagsteps,lags(1));
xc_end = vlt.data.findclosest(lagsteps,lags(end));
...
if isempty(xc_stimstim),
	xc_stimstim = vlt.signal.xcorr_stepfuncstepfunc(signal_t,stimtimes,stim,maxlag,stimtimes,stim);
...
if isempty(Rinv),
	[Rinv,R] = vlt.signal.whitening_filter_from_autocorr(xc_stimstim, length(kerneltimes));
end;
