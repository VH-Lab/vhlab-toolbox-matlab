...
% COMPUTE_LOWHALFMAX
%     [LOW,MAX,HIGH] = vlt.neuroscience.vision.oridir.indexes.compute_halfwidth( X, Y )
%
...
if( min( intfunction(1:maxv)-halfheight)>0 );
  ...
  [low,lowvalue]=vlt.data.findclosest(intfunction(1:maxv),halfheight);
  low=finex(low);
end
...
if( min( intfunction(maxv:end)-halfheight)>0 );
  ...
  [high,highvalue]=vlt.data.findclosest(intfunction(maxv:end),halfheight);
