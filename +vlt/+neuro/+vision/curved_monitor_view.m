...
alpha = vlt.math.rad2deg(2*pi*(monitor_width/2)/(2*pi*radius));



 % C - distance between straight view of animal and edge of monitor

C = 2*radius*sin(vlt.math.deg2rad(alpha/2));

phi = (180 - alpha)/2;

P = C*cos(vlt.math.deg2rad(phi));

A = sqrt(C^2-P^2);

if viewing_distance-P>0,
	theta = vlt.math.rad2deg(atan2(A,viewing_distance-P));
else,
	theta = vlt.math.rad2deg(atan2(A,viewing_distance-D));
end;

system = vlt.data.workspace2struct;
