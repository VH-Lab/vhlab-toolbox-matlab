function [isolate_indexes, isolate_indexes_in_roi] = ROI_isolate(imsize, indexesnd)
...
subs_roi = cell(1, numel(imsize));
subs_all = cell(1, numel(imsize));

[subs_roi{:}] = ind2sub(imsize, indexesnd);
...
for i=1:numel(subs_roi),
	Dmin(i) = min(subs_roi{i});
	Dmax(i) = max(subs_roi{i});
	grids{i} = Dmin(i):Dmax(i);
end

D = Dmax - Dmin + 1;
[subs_all{:}] = ndgrid(grids{:});

isolate_indexes = reshape(sub2ind(imsize, subs_all{:}), D);
isolate_indexes_in_roi = ismember(isolate_indexes, indexesnd);
