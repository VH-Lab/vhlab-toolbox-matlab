...
for i=1:length(angles) 
    ctr = (sep/2) * xy * vlt.math.rot2d((pi/180)*angles(i));
    myfilter = zeros(size(X));
    inds_1 = find(vlt.image.inside_ellipse(X,Y,ctr(1),ctr(2),rad,rad,0)); % first LED
    inds_2 = find(vlt.image.inside_ellipse(X,Y,-ctr(1),-ctr(2),rad,rad,0)); % second LED
    myfilter(inds_1) = 1; 
    myfilter(inds_2) = 1;
