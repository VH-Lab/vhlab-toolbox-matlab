classdef dumbjsondb
	...
	methods
			...
			%
				...
				switch lower(command),
					case 'new', % create a new object
						...
						if nargin>2,
							try,
								vlt.data.assign(varargin{3:end});
							catch,
		...
		function [document, doc_version] = read(dumbjsondb_obj, doc_unique_id, doc_version)
			...
			%
				doc_unique_id = vlt.file.dumbjsondb.fixdocuniqueid(doc_unique_id); % make sure it is a string
				if nargin<3,
					doc_version = [];
				end;
		...
		function [fid, key, doc_version] = openbinaryfile(dumbjsondb_obj, doc_unique_id, doc_version)
			...
			% See also: DUMBJSONDB/CLOSEBINARYFILE, DUMBJSONDB/READ, CHECKOUT_LOCK_FILE, FREAD, FWRITE
				...
				f = vlt.file.dumbjsondb.uniqueid2binaryfilename(doc_unique_id, doc_version);
				p = dumbjsondb_obj.documentpath();
				...
				if lockfid > 0,
					...
					fid = fopen([p f], 'a+', 'l'); % open in read/write mode, impose little-endian for cross-platform compatibility
			...
			% See also: REGEXPI
				...
				for i=1:numel(docids),
					...
					for j=1:numel(v_here),
						...
						b = vlt.file.dumbjsondb.ismatch(doc_here, searchParams);
						if b,
							docs{end+1} = doc_here;
		...
		function dumbjsondb_obj = remove(dumbjsondb_obj, doc_unique_id, version)
			...
			%
				...
				for i=1:numel(version),
					vfname = vlt.file.dumbjsondb.uniqueid2filename(doc_unique_id,version(i));
		...
		function v = docversions(dumbjsondb_obj, doc_unique_id)
			...
			% 
				v = [];
				fname = vlt.file.dumbjsondb.uniqueid2filename(doc_unique_id,0);
	...
	methods (Access=protected) % only available to subclasses
		...
		function writeobject(dumbjsondb_obj, doc_object, doc_unique_id, doc_version)
			...
			%
				...
				fb = vlt.file.dumbjsondb.uniqueid2binaryfilename(doc_unique_id, doc_version);
				fid=fopen([p fb],'w');
				fclose(fid);
		...
		function updatedocmetadata(dumbjsondb_obj, operation, document, doc_unique_id, doc_version)
			...
			%
				p = dumbjsondb_obj.documentpath();
				metafile = [p vlt.file.dumbjsondb.uniqueid2metafilename(doc_unique_id)];
		...
		function dumbjsondb_obj = loadparameters(dumbjsondb_obj, filename)
			...
			%
				...
				s = vlt.file.dumbjsondb.readparametersfile(dumbjsondb_obj.paramfilename);
	...
	methods (Static=true, Access=protected)
		...
		function f = uniqueid2metafilename(doc_unique_id)
			...
			%
				f = [vlt.file.dumbjsondb.uniqueid2filenameprefix(doc_unique_id) '.txt'];
		...
		function f = uniqueid2filename(doc_unique_id, version)
			...
			%
				f = vlt.file.dumbjsondb.uniqueid2filenameprefix(doc_unique_id);
				if nargin<2,
					version = 0;
				end;
				if isempty(version),
		...
		function f = uniqueid2binaryfilename(doc_unique_id, version)
			...
			%
				if nargin<2,
					f = vlt.file.dumbjsondb.uniqueid2filename(doc_unique_id);
				else,
					f = vlt.file.dumbjsondb.uniqueid2filename(doc_unique_id, version);
				end;
				f = [f '.binary'];
		...
		function b = ismatch(document, searchParams)
			...
			% Examines the fields of DOCUMENT to determine if there is a match.
				if isstruct(searchParams),
					b = vlt.data.fieldsearch(struct(document),searchParams);
					return;
				end;
				b = 1;
				for i=1:2:numel(searchParams),
					...
					if hasit,
						...
						if ischar(searchParams{i+1}), % it is a regular expression
							...
							if ~vlt.data.eqlen(value,searchParams{i+1}),
								b = 0;
							end;
						end;
