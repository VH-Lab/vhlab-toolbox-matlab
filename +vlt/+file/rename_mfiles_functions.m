...
for i=1:numel(dirstrings_to_exclude),
	...
	inds = find(vlt.data.isempty_cell(matches)); % find indexes of all matches
...
for i=1:numel(editmfiledirlist),
	for j=1:size(sublist,1),
		if 1 | ~strcmp(sublist{j}{1},sublist{j}{2}), % make sure there is something to do
			opstr = ['vlt.file.searchreplacefiles_shell(' editmfiledirlist{i} ', ' sublist{j}{1} ', ' sublist{j}{2} ');'],
			if ~strcmp(upper(sublist{j}{1}),sublist{j}{1}),
				opstr = ['vlt.file.searchreplacefiles_shell(' editmfiledirlist{i} ', ' upper(sublist{j}{1}) ', ' upper(sublist{j}{2}) ');'],
			end
		end
	end
end
