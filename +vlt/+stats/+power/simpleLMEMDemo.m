...
%   assuming a "Shuffle-First" workflow.
    ...
    targetFactor = "Drug";
    targetLevel = "DrugA";
    effectSize = 3.0; % Small-to-Medium effect size relative to noise
    ...
    stats = vlt.stats.power.simpleLMEM(T, "ReactionTime", "Animal_ID", ...
        factors, shuffleMask, targetFactor, targetLevel, effectSize, ...
    ...
    fprintf('Successes (p<.05): %d\n', stats.successCount);
    fprintf('CALCULATED POWER:  %.1f%%\n', stats.power * 100);
    fprintf('=======================================================\n');
    
    % Optional: Plot p-value distribution
    figure;
    histogram(stats.pValues, 20);
    title(['P-Value Distribution (Power = ' num2str(stats.power*100) '%)']);
    xlabel('p-value');
    ylabel('Frequency');
