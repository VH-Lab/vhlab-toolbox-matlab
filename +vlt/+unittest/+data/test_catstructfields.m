classdef test_catstructfields < matlab.unittest.TestCase
    methods(Test)
        function test_simple_concatenation(testCase)
            ...
            s1 = struct('a', 1, 'b', 2);
            s2 = struct('a', 3, 'b', 4);
            s_cat = vlt.data.catstructfields(s1, s2);
        ...
        function test_mismatched_fields(testCase)
            ...
            testCase.verifyError(@() vlt.data.catstructfields(s1, s2), 'MATLAB:vlt:data:catstructfields:mismatchedFields');
        end
        ...
        function test_dimension_argument(testCase)
            ...
            s_cat_dim1 = vlt.data.catstructfields(s1, s2, 1);
            ...
            s_cat_dim2 = vlt.data.catstructfields(s1, s2, 2);
        ...
        function test_empty_structs(testCase)
            ...
            s1 = struct('a', [], 'b', []);
            s2 = struct('a', 1, 'b', 2);
            s_cat1 = vlt.data.catstructfields(s1, s2);
            testCase.verifyEqual(s_cat1.a, 1);
            testCase.verifyEqual(s_cat1.b, 2);

            s_cat2 = vlt.data.catstructfields(s2, s1);
