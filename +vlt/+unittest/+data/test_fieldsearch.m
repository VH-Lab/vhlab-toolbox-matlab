classdef test_fieldsearch < matlab.unittest.TestCase
    ...
    methods (Test)
        function test_regexp(testCase)
            ...
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
        ...
        function test_exact_string_anycase(testCase)
            searchstruct = struct('field', 'a', 'operation', 'exact_string_anycase', 'param1', 'STRING_TEST', 'param2', '');
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyTrue(logical(result));
        ...
        function test_hassize(testCase)
            searchstruct = struct('field', 'b', 'operation', 'hassize', 'param1', [1 3], 'param2', '');
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyTrue(logical(result));

            searchstruct.param1 = [3 1];
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyFalse(logical(result));
        end

        function test_hasmember(testCase)
            searchstruct = struct('field', 'b', 'operation', 'hasmember', 'param1', 2, 'param2', '');
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyTrue(logical(result));

            searchstruct.param1 = 4;
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyFalse(logical(result));
        end
        ...
        function test_partial_struct(testCase)
            searchstruct = struct('field', 'c', 'operation', 'partial_struct', 'param1', struct('d', 5), 'param2', '');
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyTrue(logical(result));

            searchstruct.param1 = struct('d', 6);
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyFalse(logical(result));

            searchstruct.param1 = struct('e', 5);
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyFalse(logical(result));
        end
        ...
        function test_negation(testCase)
            searchstruct = struct('field', 'a', 'operation', '~exact_string', 'param1', 'not_the_string', 'param2', '');
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyTrue(logical(result));

            searchstruct.param1 = 'string_test';
            result = vlt.data.fieldsearch(testCase.test_struct, searchstruct);
            testCase.verifyFalse(logical(result));
        end
