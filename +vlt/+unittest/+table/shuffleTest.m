classdef shuffleTest < matlab.unittest.TestCase
...
% 3. Subject-level shuffle (shuffling entire subject profiles).
    ...
    methods (Test)
        ...
        function testSubjectLevelShuffle(testCase)
            ...
            % === FIX 1: Use () to extract sub-tables, not {} ===
            s1_shuffled = shuffledProfile(1:2, :);
            s2_shuffled = shuffledProfile(3:4, :);
            s1_original = originalProfile(1:2, :);
            s2_original = originalProfile(3:4, :);

            % The two shuffled profiles should be equal to the two original profiles
            % (possibly swapped).

            % === FIX 2: Use isequal() to compare tables, not all(... == ...) ===
            isS1_S1 = isequal(s1_shuffled, s1_original);
            isS1_S2 = isequal(s1_shuffled, s2_original);
            isS2_S1 = isequal(s2_shuffled, s1_original);
            isS2_S2 = isequal(s2_shuffled, s2_original);
