classdef TestKiCadGerber2OSHPark < matlab.unittest.TestCase
    % Test for the deprecated KiCadGerber2OSHPark function in the pcb/ directory
    ...
    methods (Test)
        function testFileRenamingAndContent(testCase)
            % Add the directory of the function to the path
            % This ensures that the test runner can find the deprecated function
            this_file_path = mfilename('fullpath');
            [vlt_repo_path,~,~] = fileparts(fileparts(fileparts(fileparts(fileparts(this_file_path)))));
            addpath(fullfile(vlt_repo_path, 'pcb'));

            % Run the function under test
            KiCadGerber2OSHPark(testCase.testDir);
