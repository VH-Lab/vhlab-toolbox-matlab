classdef test_numornan < matlab.unittest.TestCase
    % TEST_NUMORNAN - tests for the numornan function

    methods (Test)

        function test_numornan_basic(testCase)
            % Test case 1: Non-empty input
            testCase.verifyEqual(numornan(5), 5);
            testCase.verifyEqual(numornan([1 2]), [1 2]);

            % Test case 2: Empty input
            testCase.verifyTrue(isnan(numornan([])));
        end

        function test_numornan_with_dims(testCase)
            % Test case 1: Empty input with specified dimensions
            testCase.verifyEqual(size(numornan([], [2 3])), [2 3]);
            testCase.verifyTrue(all(isnan(numornan([], [2 3])),'all'));

            % Test case 2: Non-empty input, smaller than dims
            input = [1 2; 3 4];
            output = numornan(input, [3 3]);
            ...
            testCase.verifyError(@() numornan([1 2; 3 4], [1 1]), '');
        end
