classdef test_hashmatlabvariable < matlab.unittest.TestCase
    ...
    methods (Test)
        function test_hash_consistency(testCase)
            ...
            % Calculate hashes
            hash1 = hashmatlabvariable(data1);
            hash2 = hashmatlabvariable(data2);
            hash3 = hashmatlabvariable(data3);
        ...
        function test_different_data_types(testCase)
            ...
            hash_numeric = hashmatlabvariable(numeric_data);
            testCase.verifyClass(hash_numeric, 'char');
            testCase.verifyEqual(hash_numeric, hashmatlabvariable(numeric_data));

            % Cell array
            cell_data = {'hello', 5, [1; 2]};
            hash_cell = hashmatlabvariable(cell_data);
            testCase.verifyClass(hash_cell, 'char');
            testCase.verifyEqual(hash_cell, hashmatlabvariable(cell_data));

            % String
            string_data = "this is a string";
            hash_string = hashmatlabvariable(string_data);
            testCase.verifyClass(hash_string, 'char');
            testCase.verifyEqual(hash_string, hashmatlabvariable(string_data));

            % Char
            char_data = 'this is a char';
            hash_char = hashmatlabvariable(char_data);
            testCase.verifyClass(hash_char, 'char');
            testCase.verifyEqual(hash_char, hashmatlabvariable(char_data));
