classdef testmlstr2var < matlab.unittest.TestCase
    % TESTMLSTR2VAR - test the mlstr2var function
    ...
    methods (Test)
        function testSimpleCases(testCase)
            % Test empty string
            testCase.verifyEqual(mlstr2var(''), []);

            % Test simple numeric value
            testCase.verifyEqual(mlstr2var('123'), 123);

            % Test numeric matrix
            testCase.verifyEqual(mlstr2var('[1 2; 3 4]'), [1 2; 3 4]);

            % Test char string
            testCase.verifyEqual(mlstr2var('''hello world'''), 'hello world');
        end

        function testCellArray(testCase)
            % Use cell2mlstr to generate a valid string
            c_original = {1, 'two', [3 3]};
            ml_str = cell2mlstr(c_original);

            c_reconstructed = mlstr2var(ml_str);
        ...
        function testStruct(testCase)
            ...
            ml_str = struct2mlstr(s_original);

            s_reconstructed = mlstr2var(ml_str);
        ...
        function testNestedData(testCase)
            ...
            [s_reconstructed] = testCase.verifyWarning(@() mlstr2var(ml_str), 'MATLAB:colon:operandsNotRealScalar');
