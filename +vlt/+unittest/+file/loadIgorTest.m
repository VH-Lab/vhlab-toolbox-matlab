classdef loadIgorTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function test_loadIgor_no_file(testCase)
            ...
            testCase.verifyError(@() vlt.file.loadIgor(non_existent_file), '', 'Expected an error for a non-existent file.');
        end
        ...
        function test_loadIgor_wrong_nargin(testCase)
            ...
            testCase.verifyError(@() vlt.file.loadIgor('dummy_file.bin', 1), '', 'Expected an error for 2 input arguments.');
        end

        % Note to user: A full test of vlt.file.loadIgor requires a sample Igor binary file.
