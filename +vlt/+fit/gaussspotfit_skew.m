function [mu, C, amp, alpha, fit_responses] = gaussspotfit(xrange, yrange, x_ctr, y_ctr,radius,response)
...
Upper = [ max(xrange); max(yrange); 10*max(radii)^2; 10*(max(radii)^2); 10*max(radii)^2; Inf; 0.15; 0.15 ];
Lower = [ min(xrange); min(yrange); 0; -10*(max(radii)^2); 0;0;-0.15;-0.15];
StartPoint = [ mu(1); mu(2); C(1);C(2);C(4); max(amp_initial); 0; 0];

x = lsqcurvefit(@(x,xdata) vlt.math.ellipse_on_mvnskewpdf_x0(x,xdata,X,Y),StartPoint,ellipse_params,response,Lower,Upper);

fit_responses = vlt.math.ellipse_on_mvnskewpdf_x0(x,ellipse_params,X,Y);
