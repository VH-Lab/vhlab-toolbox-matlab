...
for i=list,
   ...
   if ~isempty(ext2str), try, ex2 = evalin('base',ext2str);
	catch, error(['Could not find extractor ' ext2str '.']);end;
	[ex2,status]=setupextract(ex2,cksds.nameref_list(i),cksds,instruc);
	if status<0,error(['Error in setup; extractor: ' ...
...
for i=list,
   ...
   if ~isempty(ext2str), try, ex2 = evalin('base',ext2str);
	...
	[ex2,status]=cleanupextract(ex2,cksds.nameref_list(i),cksds,instruc);
	if status<0,error(['Error in cleanup ; extractor: ' ...
		ext2str ', nameref: ' cksds.nameref_list(i).name ',' ...
		int2str(cksds.nameref_list(i).ref) '.']); end;
	assignin('base',ext2str,ex2);
