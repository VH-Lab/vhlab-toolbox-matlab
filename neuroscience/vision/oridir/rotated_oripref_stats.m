...
%
% [MEANANGLE,DISPERSION] = ROTATED_ORIPREF_STATS(ORI_PREF, REFERENCE_ANGLE)
%
...
ori_pref_rotated = angdiffwrapsign(ori_pref - ref_angle, 180);

ori_pref2 = 2*ori_pref_rotated; % now stretch ori space to 0..360, doubled orientation space

mu = circ_mean(deg2rad(ori_pref_rotated(:)));  % in 0..360, doubled orientation space

meanangle = rad2deg(mu); % wrap back to 0..180

dispersion = rad2deg(max(circ_std(deg2rad(ori_pref2(:))),0));  % deal with round-off
