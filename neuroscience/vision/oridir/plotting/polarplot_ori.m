...
%
%  [H,OUTPUTS]=POLARPLOT_ORI(ANGLES, RESPONSES, ...)
%
...
%
%  See also: DIRSPACE2ORISPACE, MMPOLAR, POLARPLOT_DIR
%
...
hh=mmpolar(2*deg2rad(angles([1:end 1])),responses([1:end 1]),...  % connects the last point to the first
	'k-o', ...
	'style',style,...
...
if showmeanvector,
	hold on;
	outputs.h_meanvector = mmpolar(2*deg2rad(outputs.vectorpref)*[1 1], [0 outputs.vectormag],...
		'b-^', ...
		'style', style);
end;
...
if showcircularvariance,
	hold on;
	outputs.h_circularvariance= mmpolar(2*deg2rad(outputs.vectorpref)*[1 1], [0 1-outputs.circularvariance],...
		'g-s', ...
		'style', style);
end;
