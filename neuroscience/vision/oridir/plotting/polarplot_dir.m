...
%
%  [H,OUTPUTS]=POLARPLOT_DIR(ANGLES, RESPONSES, ...)
%
...
%
%  See also: DIRSPACE2ORISPACE, MMPOLAR, POLARPLOT_ORI
%
...
hh=mmpolar(deg2rad(angles([1:end 1])),responses([1:end 1]),...  % connects the last point to the first
	'k-o', ...
	'style',style,...
...
outputs.meanvector = sum(responses(:).*exp(sqrt(-1)*mod(deg2rad(angles(:)),2*pi)));
outputs.vectormag = abs(outputs.meanvector);
%outputs.vectorpref = cartesian2compass(rad2deg(mod(angle(outputs.meanvector),2*pi)),0); no, the plotting function mmpolar will plot it properly
outputs.vectorpref = rad2deg(mod(angle(outputs.meanvector),2*pi));
outputs.dircircularvariance = compute_dircircularvariance(angles(:)',responses(:)');
[outputs.dvco_di,outputs.dvco_pref,outputs.dvco_vec] = compute_dirvecconstrainedori(angles(:)',responses(:)');
outputs.h_meanvector = [];
outputs.h_circularvariance = [];

if showmeanvector,
	hold on;
	outputs.h_meanvector = mmpolar(deg2rad(outputs.vectorpref)*[1 1], [0 outputs.vectormag],...
		'b-^', ...
		'style', style);
end;
...
if showdircircularvariance,
	hold on;
	outputs.h_circularvariance= mmpolar(deg2rad(outputs.vectorpref)*[1 1], [0 1-outputs.dircircularvariance],...
		'g-s', ...
		'style', style);
end;
