...
ot_vec = rates*transpose(exp(sqrt(-1)*2*mod(deg2rad(angles),pi)));
ot_vec = exp(sqrt(-1)*angle(ot_vec)/2);
ot_angle = mod(rad2deg(angle(ot_vec)),360);
...
if 0~=(sum(angles_oneway) + sum(angles_otherway) - length(angles_rotated)),
	disp(['compute_dirvecconstrainedori error: it happened; we did not divide direction space evenly.']);
	keyboard;
end;
