...
%
% VI = ORIDIR_VECTORINDEXES(RESPSTRUCT)
%
...
% tuning_width                    |   Vector tuning width (see help COMPUTE_TUNINGWIDTH)
...
for i=1:numel(respstruct.ind),
	...
	allresps(:,i) = colvec(respstruct.ind{i}(indshere(1:smallest_n)));
end;
...
if size(allresps,1)>0,
	...
	[h2,vi.ot_HotellingT2_p]=hotellingt2test([real(vecresp_ot) imag(vecresp_ot)],[0 0]);
	...
	if hasdirection,
		...
		[h3,vi.dir_HotellingT2_p]=hotellingt2test([real(vecdirresp) imag(vecdirresp)],[0 0]);
...
vi_ot_index = compute_orientationindex(tuneangles,tuneresps);
...
if hasdirection,
	...
	vi.dir_index = compute_directionindex(angles,resp(2,:));
end;
