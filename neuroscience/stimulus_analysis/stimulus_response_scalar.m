		...
		if freq_response_here==0,
			...
			if ~isspike,
				...
				if numel(stimulus_samples)>0,
					response_here = fouriercoeffs_tf_spikes(timestamps(stimulus_samples)-stim_onsetoffsetid(i,1), ...
				...
				if numel(control_stimulus_samples)>0,
					control_response_here = fouriercoeffs_tf_spikes(timestamps(control_stimulus_samples)-stim_onsetoffsetid(control_stim_here,1),...
			...
			if ~isempty(prestimulus_time),
				if ~isspike,
					...
					if numel(prestimulus_samples)>0,
						prestimulus_here = fouriercoeffs_tf_spikes(timestamps(prestimulus_samples)-stim_onsetoffsetid(i,1)-prestimulus_time, ...
