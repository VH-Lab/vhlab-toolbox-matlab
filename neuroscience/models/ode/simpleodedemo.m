...
%
%  SIMPLEODEDEMO - Popups up a window that allows the user to watch
...
if isempty(fig),
	z = findobj(allchild(0),'flat','tag','simpleodedemo');
	if isempty(z),
		fig = figure;
	else,
		fig = z;
...
switch command, % now process the command
	case 'Main',
		set(fig,'userdata',ud);
		simpleodedemo('command','NewWindow','fig',fig);
	case 'NewWindow',
		myuiinit;
		...
		set(fig,'position',[50 50 right top],'tag','simpleodedemo','name','simpleodedemo',...
			'numbertitle','off');
	...
	case 'SimulateBt',
		[r0,tau,C,I,Itype,dT,Duration] = simpleodedemo('command','GetVars','fig',fig);
		t = 0;
		r = r0;
		Inp = I;
