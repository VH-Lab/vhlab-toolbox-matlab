...
classdef HHsynclass < neuronmodelclass
	...
	methods
		function HHobj = HHsynclass(varargin)
			V_threshold = -0.015;
			E_Na = 0.045;
			E_K = -0.082; 
			G_L = 30e-9;
		...
		function [dsdt, Itot] = dsdt(HHobj,S_value)
			...
			if HHobj.NMDA,
				I_NMDA = nmda_voltage_gate(Vm)*HHobj.Total_NMDA_G(i)*(HHobj.E_ESyn - Vm);
			else,
				I_NMDA = 0;
			end;
		...
		function HHobj = setup_synapses(HHobj)
			...
			[HHobj.Total_AMPA_G,p1] = probabilistic_release(...
			...
			[Total_AMPA_G2,p2] = probabilistic_release(...
				't_start', HHobj.t_start, 't_end', HHobj.t_end, 'dt', HHobj.dt, ...
				'facilitation_increase', HHobj.facilitation, ...
				'facilitation_tau', HHobj.facilitation_tau, ...
			...
			HHobj.Total_NMDA_G = probabilistic_release(...
			...
			HHobj.Total_NMDA_G = HHobj.Total_NMDA_G + probabilistic_release(...
				'Released_times', p2.Released_times, ...
				'Q',HHobj.GLUT_PNQ(3) * 0.50, ... % NMDA will be 50% of AMPA
				'syn_tau1',0.005, 'syn_tau2', 0.050 ... % NMDA kinetics
				);

			HHobj.Total_GABA_G = probabilistic_release(...
				't_start', HHobj.t_start, 't_end', HHobj.t_end, 'dt', HHobj.dt, ...
