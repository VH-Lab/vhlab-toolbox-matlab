...
%  array.
%  [MODELELRUNSTRUCT,VARS] = MODELELRUN_EXECUTABLE(MODELEL, PARAM1NAME, PARAM1VALUE, ...)
%
...
modelel_out = modelel_convertforc(modelrunstruct.Model_Initial_Structure, Variables);
base_matlab_dir = pathstr2cellarray(userpath);

modelel_out_string = struct2mlstr(modelel_out);

% write text output file here
infile = [base_matlab_dir{1} filesep 'Modelel_model.txt'],
cellstr2text(infile,{modelel_out_string});
...
model_in_str = textfile2char(finalstateoutfile);

modelrunstruct.Model_Final_Structure = modelel_convertfromc(model_in_str);

if nargout>1,
	varsout = modelel_importvariables(outfile,modelrunstruct.Model_Final_Structure,Variables);
else,
	varsout = [];
end;
