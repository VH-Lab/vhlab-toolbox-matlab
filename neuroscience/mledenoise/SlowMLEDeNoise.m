...
%
%   [RESPONSES, SIN_WEIGHTS] = SLOWMLEDENOISE(STIM_ONSETS, STIMOFFSETS, STIMVALUES, ...
...
%           = SLOWMLEDENOISE(STIM_ONSETS, STIMOFFSETS, STIMVALUES, ...
%	          SPIKE_TIMES, [PARAM1, VALUE1, ...]);
%
%   FIT_SIN4 is a set of 13 coefficients to a 4th order sinusoidal fit (see SIN4_FIT), 
...
%   
%
%  Examples:  see TESTSLOWMLE and TESTSLOWMLE_MODULATION
%          ('type testSlowMLE'  and 'type testSlowMLE_modulation')
%  
%  See also:  FIT_SIN4
...
smooth_spikecounts = spiketimes2bins(spike_times,smooth_time);
...
if verbose,
	disp(['SlowMLEDeNoise: about to perform sin4_fit']);
	verbfig = figure;
...
[sinfit_smooth_spikerate,sinparams] = sin4_fit(smooth_time,smooth_spikerate,maxFrequency,0);

if verbose, disp(['SlowMLEDeNoise: sin4_fit done']); end;
...
if verbose, disp(['SlowMLEDeNoise: about to fit']); end;


  % optionally, could include showfitting code at part 1 below
[s_i_current, fval] = MLE_PoisGauss(stim_center_times,stim_durations,responses_observed,...
...
for zz = 1:numIterations,
	if verbose, disp(['SlowMLEDeNoise: beginning iteration ' num2str(zz) ]); end;
	[g_current,fval]=MLE_PoisGauss(stim_center_times,stim_durations,responses_observed,...
...
if showfitting,
  ...
  %% part 1
		if showfitting,
			...
			hold on;
			RR = response_gaindrift_model(stim_center_times,stim_durations,stimlist, sinparams,g_current,s_i_initial);
			h2 = plot(stim_center_times, RR,'color',colors(1+mod(plotind,size(colors,1)),:));
			plotind = plotind + 1;
			pause(1); drawnow;
			mle(1,3) = -loglikelihood_spiketrain(RR,stim_durations,spikecounts_observed);
		end;
  %% part 2
		if showfitting,
				RR = response_gaindrift_model(stim_center_times,stim_durations,stimlist,sinparams,g_current,s_i_current);
				h2 = plot(stim_center_times, RR,'color',colors(1+mod(plotind,size(colors,1)),:));
				plotind = plotind + 1;
				pause(1); drawnow;
				mle(1,2) = -loglikelihood_spiketrain(RR,stim_durations,spikecounts_observed);
		end;
			if showfitting,
				RR = response_gaindrift_model(stim_center_times,stim_durations,stimlist,sinparams,g_current,s_i_current);
				h2 = plot(stim_center_times, RR,'color',colors(1+mod(plotind,size(colors,1)),:));
				plotind = plotind + 1;
				pause(1); drawnow;
				mle(zz+1,3) = -loglikelihood_spiketrain(RR,stim_durations,spikecounts_observed);
			end;
