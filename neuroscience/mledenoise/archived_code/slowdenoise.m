...
[x fval] = MLE_Pois(t,dt,spiketimets,stimulitimets,sinparams,fixA,1,x0,smoothness);

for zz = 1:10
	x0 = fixA;
	fixF = x;
	if F==0,
		[x fval] = MLE_Pois(t,dt,spiketimets,stimulitimets,sinparams,fixF,0,x0,smoothness);
	else,
		[x fval] = MLE_Gauss(t,dt,F1Magnitude,stimulitimets,sinparams,fixF,0,x0,smoothness);
	end;
	mle(zz,1) = fval;
	...
	if F==0,
		...
		[x fval] = MLE_Gauss(t,dt,F1Magnitude,stimulitimets,sinparams,fixA,1,x0,smoothness);
	end;
	mle(zz,2) = fval;
end
