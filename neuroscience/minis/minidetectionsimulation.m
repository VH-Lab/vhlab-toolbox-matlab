...
%
%  [OPT_THRESHOLD, DETECTOR, CONV_SCALE, STATS] = MINIDETECTIONSIMULATION
%
...
[mt,t,params] = minitemplates('Max_Amplitude',3,'Amplitude_Steps',1,'Tau_Onsets',Tau_Onset,'Tau_Offsets',Tau_Offset,'dt',dt);
...
for j=1:length(myevent_samples),
	noisytemplate=minitemplates('dt',dt,'Amplitude_Steps',1,...
...
stats = emptystruct('threshold','false_positive_rate','true_positive','false_negative','total_error_rate');

for th=1:length(Thresholds),

	[dummy,dummy2,z]=threshold_crossings_epochs(matched,Thresholds(th));
	out_times = refractory(Tvals(z),refractory_period);
	fp = [];
	found = [];
