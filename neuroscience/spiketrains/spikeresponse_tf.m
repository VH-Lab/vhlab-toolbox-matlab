...
%
%  [RESPONSE_CURVE] = SPIKERESPONSE_TF(STIMTIMES_ON, STIMTIMES_OFF, STIM_VALUES,
...
for i=1:length(stimtimes_on),
	...
	T = 0:binsize:dt;
	spikecounts = spiketimes2bins(spiketimes-stimtimes_on(i),T);
	if frequency==0, 
		f = sum(spikecounts)/dt;
	else,
		f = fouriercoeffs_tf(spikecounts(:)/binsize,... % dividing by binsize converts to rate
				frequency,...
