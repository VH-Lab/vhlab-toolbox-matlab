...
%
%  [Pxx,Fxx, BANDVALUES, BANDS] = SPIKETRAIN_POWERSPECTRUM_WINDOW(SPIKETRAIN,...
...
%
%  The power spectrum is calculated by calling the function SPIKETRAIN_POWERSPECTRUM.
%
...
%  
%
%  See also: SPIKETRAIN_POWERSPECTRUM
%
%  
...
for i=1:length(start_times),
	...
	[Pxx_here, dummy, Fxx] = spiketrain_powerspectrum(z,BINSIZE,'NFFT',NFFT);
	Pxx(:,i) = Pxx_here;
	if i==1,
		for j=1:size(bands,1),
			for k=1:2,
				bandinds(j,k) = findclosest(Fxx,bands(j,k));
			end;
		end;
	end;
	for j=1:size(bands,1),
