function varargout = cluster_spikewaves_trivial(varargin)
% CLUSTER_SPIKEWAVES_TRIVIAL - Cluster spikewaves into a single 'multi-unit' group
%
%   [CLUSTERIDS,CLUSTERINFO] = CLUSTER_SPIKEWAVES_TRIVIAL('WAVES', WAVES, ...
%      'WAVEPARAMETERS', WAVEPARAMETERS, ...)
%
%   Classifies all spikes in the manner of CLUSTER_SPIKEWAVES_GUI, except
...
%   (this is the same as the output of HEADER in READVHLSPIKEWAVEFORMFILE)
%
...
fig = '';                 % the figure
success = 0;
features = struct('name','2points','code','ud.F=transpose(spikewaves2Npointfeature(ud.waves,ud.spikewaves2NpointfeatureSampleList));','FeatureText','2 points');
features(2) = struct('name','pca3','code','ud.F=transpose(spikewaves2pca(ud.waves,3,ud.spikewaves2pcaRange));','FeatureText','range [-S0 S1]');
...
clusterids = ones(1,numel(waves));
clusterinfo = struct('number',int2str(1),'qualitylabel','Multi-unit',...
	'number_of_spikes',length(clusterids),'meanshape',nanmean(waves(:,:,:),3),...
                                        'EpochStart',EpochNames{1},'EpochStop',EpochNames{end});

varargout{1} = clusterids;
varargout{2} = clusterinfo;
...
%					'meanshape',nanmean(ud.waves(:,:,find(isnan(ud.clusterids))),3),'EpochStart',ud.EpochNames{1},'EpochStop',ud.EpochNames{end});
		...
		cluster_spikewaves_gui('command','NewWindow','fig',fig);
		...
		if isempty(ud.clusterinfo),
			cluster_spikewaves_gui('command','InitClusterInfo','fig',fig);
		end;
		cluster_spikewaves_gui('command','FeatureMenu','fig',fig);
		cluster_spikewaves_gui('command','EnableDisableEpochItems','fig',fig);
		drawnow;
		if ClusterRightAway,
			cluster_spikewaves_gui('command','ClusterAllBt','fig',fig);
		end;
		if ~ud.IsModal, 
			return;
	...
	case 'NewWindow',
		...
		row = ud.windowrowheight;
                ...
                set(fig,'position',[50 50 right top],'tag','cluster_spikewaves_gui');
	...
	case 'ShowInitialClustersBt',
		cluster_spikewaves_gui('waves',ud.waves,'waveparameters',ud.waveparameters,'clusterids',ud.clusterids_initial,'clusterinfo',ud.clusterinfo_initial,...
	...
	case 'FeatureEdit',
		...
		if good, cluster_spikewaves_gui('command','FeatureMenu','fig',fig); end;
	case 'FeatureMenu',
		...
		if good,
			...
			set(fig,'userdata',ud);
			cluster_spikewaves_gui('command','Draw','fig',fig);
		end;
	case 'ClusterAllBt',
		...
		if good,
			set(fig,'userdata',ud);
			cluster_spikewaves_gui('command','ReOrderMinToMax','fig',fig);
			cluster_spikewaves_gui('command','InitClusterInfo','fig',fig);
			cluster_spikewaves_gui('command','MakeClusters1toN','fig',fig);
			cluster_spikewaves_gui('command','Draw','fig',fig);
		end;
	case 'ClusterVisibleBt',
		...
		if good,
			...
			indexes = cluster_spikewaves_gui('command','FindVisibleSpikes','fig',fig);
...
%algorithms(2) = struct('name','KMeans','code','ud.clusterids=kmeans(ud.F,ud.clusters);','ClusterSizeEdit','5','AlgorithmSizeTxt','# clusters:');
	...
	case 'InitClusterInfo',
		...
		set(fig,'userdata',ud);
		cluster_spikewaves_gui('command','UpdateMergeQualityMenu','fig',fig);
	...
	case 'OtherActionMenu',
		...
		if ~isempty(commands{v}), 
			cluster_spikewaves_gui('command',commands{v},'fig',fig);
		end;
	case 'SplitCluster', % requires input 'indexes', 'parentcluster' 
		...
		set(fig,'userdata',ud);
		cluster_spikewaves_gui('command','MakeClusters1toN','fig',fig); % possible we emptied one
		cluster_spikewaves_gui('command','UpdateMergeQualityMenu','fig',fig);
		cluster_spikewaves_gui('command','Draw','fig',fig);
	...
	case 'SelectSpikesToAddFeatureAxes',
		currentSelectedCluster = cluster_spikewaves_gui('command','CurrentSelectedCluster','fig',fig);
	...
	case 'SelectSpikesToExcludeFeatureAxes', 
		...
		z = find(inside); % who is inside the selection region?
		indexes = indexes(z); 
		cluster_spikewaves_gui('command','SplitCluster','fig',fig,'parentcluster',currentSelectedCluster,'indexes',indexes);
	case 'ExcludeAllVisibleSpikes',
		parentcluster = cluster_spikewaves_gui('command','CurrentSelectedCluster','fig',fig);
		indexes = cluster_spikewaves_gui('command','FindVisibleSpikes','fig',fig);
		if ~isnan(parentcluster),
			...
			indexes = indexes(z); 
			cluster_spikewaves_gui('command','SplitCluster','fig',fig,'parentcluster',parentcluster,'indexes',indexes);
		else,
	...
	case 'NotPresentIntoNewCluster',
		...
		[inds_present,dummy,inds_notpresent] = samplesinepochs(indexes, ud.EpochStartSamples, ...
		...
		cluster_spikewaves_gui('command','SplitCluster','fig',fig,'parentcluster',parentcluster,'indexes',inds_notpresent);
	case 'UpdateMergeQualityMenu',  % update the labels in the Merge/Quality/Epoch menus
		...
		cluster_spikewaves_gui('command','Merge1Menu','fig',fig);
	case 'MoveTo1Menu',
	...
	case 'Merge1Menu',  % Set Merge1Menu value
		...
		cluster_spikewaves_gui('command','ClusterPropertyMenu','fig',fig);
	case 'MergeBt',
		mm1 = findobj(fig,'tag','Merge1Menu');
		mm2 = findobj(fig,'tag','Merge2Menu');
		v1 = get(mm1,'value');
	...
	case 'ClusterPropertyMenu',
		...
		cluster_spikewaves_gui('command','UpdateEpochMenus','fig',fig);
	case 'QualityMenu',
		...
		set(fig,'userdata',ud);
		cluster_spikewaves_gui('command','RelabelSpikes','fig',fig);
	...
	case 'EnableDisableEpochItems',
		...
		for i=1:length(itemlist), 
			set(findobj(fig,'tag',itemlist{i}),'visible',onoff(length(ud.EpochStartSamples)>1));
		end;
	case 'MarkerSizeEdit',
		MarkerSizeString = get(findobj(fig,'tag','MarkerSizeEdit'),'string');
		ud.MarkerSize = str2num(MarkerSizeString);
		set(fig,'userdata',ud);
		cluster_spikewaves_gui('command','DrawFeatures','fig',fig);
	case {'RandomSubsetSizeEdit', 'RandomSubsetCB'},
		...
		set(fig,'userdata',ud);
		cluster_spikewaves_gui('command','DrawSpikes','fig',fig);
	case 'AxesEdit', % user modified axes values or we want to initiate same command
		...
		if FixedAxesCB,
			try,
				...
				cluster_spikewaves_gui('command','AxesAutoBt','fig',fig); % repair the damage
			end;
		end;
	...
	case 'FindVisibleSpikes', % spikes that are currently being viewed in the epoch, uses input variable 'indexes' if available
		...
		[inds_visible,dummy,inds_notvisible] = samplesinepochs(indexes, ud.EpochStartSamples, viewepochsstartvalue, viewepochsstopvalue);
	...
	case 'DrawFeatures',
		...
		for i=1:length(clusters),
			...
			[inds_visible,inds_invisible] = cluster_spikewaves_gui('command','FindVisibleSpikes','fig',fig,'indexes',inds);
			inds = inds_visible; 
			% and also restrict for spikes that are 'present' in the cluster
			[inds_present,dummy,inds_notpresent] = samplesinepochs(inds, ud.EpochStartSamples, ...
		...
		cluster_spikewaves_gui('command','AxesEdit','fig',fig); % adjust axis if necessary
		drawnow;
	case 'DrawSpikes',
		...
		[inds_visible,inds_invisible] = cluster_spikewaves_gui('command','FindVisibleSpikes','fig',fig);
		...
		for i=1:length(clusters),
			...
			inds_present = samplesinepochs(inds, ud.EpochStartSamples, ...
				find(strcmp(ud.clusterinfo(clusteridhere).EpochStart,ud.EpochNames)) , ...
				find(strcmp(ud.clusterinfo(clusteridhere).EpochStop,ud.EpochNames)) );
			inds = inds_present;
			if mod(i,2),
			...
			plotspikewaves(ud.waves(:,:,inds),1:length(inds),'ColorOrder',thecolor,'RandomSubset',ud.RandomSubset,'RandomSubsetSize',ud.RandomSubsetSize);
			box off;
			axis off;
			A = [A ; axis ];
		end;
	...
	case 'RelabelSpikes',
		for i=1:length(ud.clusterinfo),
			...
			inds = samplesinepochs(inds, ud.EpochStartSamples, get(findobj(fig,'tag','ViewEpochsStartMenu'),'value'), get(findobj(fig,'tag','ViewEpochsStopMenu'),'value'));
                        inds = samplesinepochs(inds, ud.EpochStartSamples, ...
		...
		if b | ~ud.ForceQualityAssessment,
			...
			if saidyes,
				...
				for i=1:length(ud.clusterinfo),
					...
					[inds,dummy,inds_notpresent] = samplesinepochs(inds, ud.EpochStartSamples, ...
		                                find(strcmp(ud.clusterinfo(i).EpochStart,ud.EpochNames)) , ...
						find(strcmp(ud.clusterinfo(i).EpochStop,ud.EpochNames)) );
