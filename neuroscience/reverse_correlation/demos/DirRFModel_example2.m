...
%    (Note that there are no spikes generated here, that is DIRRFMODEL_EXAMPLE3)
...
SpFreq = 1; TFreq = 4; Left = 1; Right = -1; Stim_Center_Position = 3;
  ...
  % make the kernel
D = createdirkernel(Dx,Dt,SpFreq,0,TFreq,Left,[Stim_Center_Position 2],[0.1 0.001 0.2],0.100);
...
alpha = 0.4;
stim_random = stim1d_random(Stimx,Stimt,100*[1 0 -1],[alpha 1-2*alpha alpha]);
...
Rspikerate_random = 50*rectify(R_random);  % assume units of Hz, 1Hz/mV
...
stim_autocorrelation = step_autocorrelation((100*100)*alpha, N, 0:length(kerneltimes));
...
[computed_kernel, computed_kernel_unfiltered, correlation_stimulus_response, correlation_stimulus_stimulus] = ...
		reverse_correlation_mv_stepfunc(R_random,sim_time,kerneltimes,Stimt,stim_random,...
...
column_with_most_kernel = findclosest(Stimx,Stim_Center_Position);
