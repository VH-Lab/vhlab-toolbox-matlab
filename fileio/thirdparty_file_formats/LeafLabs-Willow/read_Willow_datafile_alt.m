...
if channel_type == 2, % amplifier data, 30 khz
    ...
    force_single_channel_read = 1;

    [chan_sort, sort_indexes] = sort(channel_numbers);
    consecutive_channels_requested = (length(channel_numbers)==1) | (all(diff(chan_sort)==1));

    s0 = 1+(time_indices(1)-1)*1024; % this is the first sample in the block
    sample_count_per_channel = time_indices(end) - time_indices(1) + 1;

    data = zeros(length(channel_numbers),sample_count_per_channel);

    if ~force_single_channel_read & consecutive_channels_requested, 
	  s00 = s0 + (chan_sort(1)-1);
          skip = 1024 - (chan_sort(end)-chan_sort(1));
          % read a burst of samples from consecutive channels
          error('not implemented yet.');
          if ~all(chan_sort==channel_numbers),
                data = h5read(filename,'/channel_data', s00, sample_count_per_channel, skip);
          end;
    else, % read a single channel at a time
          skip = 1024;
          for c=1:length(channel_numbers),
		s00 = s0 + (channel_numbers(c)-1);
		data(c,:) = h5read(filename,'/channel_data', s00, sample_count_per_channel, skip);
          end;
    end;
...
data = data;  % column for each channel
