...
%
%   SYNAPSEEL = SYNAPSEEL_STDP(SYNAPSEEL, MODELSTRUCT)
%
...
prespike = isspiking(modelstruct(synapseel.model.pre));
postspike = isspiking(modelstruct(synapseel.model.post));
...
if prespike|postspike,
	...
	if synapseel.model.plasticity_params.classic_stdp,
		dW = stdp_apply( modelstruct(synapseel.model.pre).model.spiketimes(1:tpre_exclude-1),...
		...
		dW = stdp_triplet_apply( modelstruct(synapseel.model.pre).model.spiketimes(1:tpre_exclude-1),...
			 modelstruct(synapseel.model.post).model.spiketimes(1:tpost_exclude-1),...
			'T0',Tnow-eps,synapseel.model.plasticity_params.params{:});
	end;
