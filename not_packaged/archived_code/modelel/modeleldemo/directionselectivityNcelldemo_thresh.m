...
% % protosynffI = synapset1t2el_init('plasticity_params',plasticity_params_ffinhib,...    % is this to make ff connetions inputs --> inhib?
...
% % 
% % protosyn_inhib = synapset1t2el_init('plasticity_params',plasticity_params_inhib,... % is this to make syn inhib --> output??
% % 			'plasticity_method',plasticity_method_inhib,synapseparams_inhib{:},'dT',dt);

mel(end+1) = intfireleakyel_init('name','output',intfireparams{:},'dT',dt,'V_threshold',V_threshold);
output_cell = length(mel);


mel = modelelsynconn(mel,1:((N*R)+1),[nan(N*R+1,N*R) [mask(:).*Syn_Gmax_initial'; NaN] ], protosyn);
